...
コンソールで直接 `js-analyzer` を実行してください。プロジェクトディレクトリに入る必要はありません。プロジェクトはページ上で直接追加・設定できるからです。
...
その後、http://localhost:8088/ にアクセスして分析ページを開きます。

### 3. ページ上でプロジェクトを手動追加

1. ヘッダー右上の歯車アイコン（⚙️）をクリックし、【プロジェクト管理】ダイアログを開きます。
2. 【手動でプロジェクトを追加】をクリックし、右側の JSON エディタにプロジェクト設定を入力します。
   - `name`：プロジェクト名
   - `root`：プロジェクトルートディレクトリの絶対パス（必須）
   - `ignore`：分析不要なディレクトリ。例 `["**/node_modules/**", "**/dist/**"]`
   - `extensions`：実際に使用するソースファイルの拡張子。例 `[".js", ".ts", ".tsx", ".vue", ".json", ".jsx"]`
   - `alias`：パスのエイリアスマッピング。例 `{ "@/": "/src/" }`
   - `ide`：ファイルを開くために使用する IDE。デフォルトは `"code"`
3. 【設定を保存】をクリックすると、サービスが自動的にプロジェクトを分析し、依存関係情報を表示します。
4. 複数のプロジェクトを追加でき、ヘッダーのプロジェクトドロップダウンで現在のプロジェクトを切り替えられます。各プロジェクトは【プロジェクト管理】で更新（再分析）または削除することもできます。

> 💡 [AI によるプロジェクト設定の生成](#ai-によるプロジェクト設定の生成) を利用すれば、設定を手で書く必要はありません。

> ⚠️ ローカルインストールは推奨しなくなりました。プロジェクトはページ上で直接追加・設定できるため、1 回のグローバルインストールですべてのプロジェクトをまかなえます。

## 設定ファイル

上記コマンドで分析サービスを迅速に起動することができますが、プロジェクトの全体的なアーキテクチャが異なるため、js-analyzer がより正確に分析するためには、いくつかの必要な情報を設定する必要があります。（これは任意です。同じ設定はページの【プロジェクト管理】で直接編集することもできます。）

設定ファイルを指定するには、`--config` オプションを付けてサービスを起動します。
...
新しいプロジェクトを追加する際に、[js-analyzer-config-prompt.md](./js-analyzer-config-prompt.md) 内の中国語プロンプトを使用すると、プロジェクト設定を自動的に生成できます。

1. プロンプトファイルを開いて内容をコピーします。
2. プロンプトを AI アシスタントに貼り付け、分析対象のプロジェクトの絶対パスを渡します。
3. 返ってきた JSON をプロジェクト設定フォームにコピーするか、`js-analyzer.js` に合わせて調整します。

このプロンプトは、AI にプロジェクトの package メタデータ、TypeScript / JavaScript 設定、ビルド設定、無視ルール、ソースファイルの拡張子を調査させ、応答には JSON 設定オブジェクトのみを含めることを要求します。
